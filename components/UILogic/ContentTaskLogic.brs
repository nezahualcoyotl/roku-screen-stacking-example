sub RunContentTask()
    m.contentTask = CreateObject("roSGNode", "MainLoaderTask") ' create tasj for feed retrieving
    ' obvserve content so we can know when feed content will be parsed
    ...
    m.loadingIndicator.visible = true ' show indicator while content is loading
end sub

sub OnMainContentLoaded() ' invoked when content is ready to be used
    m.GridScreen.SetFocus(true) ' set focus to GridScreen
    m.loadingIndicator.visible = false ' hide loading indicator because content was retreived
